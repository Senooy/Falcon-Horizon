...
const Tableau = () => {
  ...
  const [filter, setFilter] = useState({ period: "Tous", status: "Tous", showConnectingDate: false });
  ...
  const sortRecords = (records) => {
    return records.sort((a, b) => {
      ...
      if (valueA instanceof Date && valueB instanceof Date) {
        return sortConfig.ascending ? valueB - valueA : valueA - valueB;
      }
  ...
  const filterRecords = (period, status, showConnectingDate) => {
    const now = new Date();
    ...
    const filteredByConnectingDate = showConnectingDate
      ? filteredByStatus.filter((record) => !!record.ConnectingDatePlanned__c)
      : filteredByStatus;
  
    return filteredByConnectingDate;
  };
  
  

  const handleFilter = (period, status, showConnectingDate) => {
    const filtered = filterRecords(period, status, showConnectingDate);
    const sortedFiltered = sortRecords(filtered);
    setFilteredRecords(sortedFiltered);
    setFilter({ period, status, showConnectingDate }); // Ajout de showConnectingDate
  };
    ...
    >
      ...
      <Flex direction={{ base: "column", md: "column" }} w="100%" alignItems={{ base: 'left', md: 'left' }}>
        ...
        <Box mb={4}>
          <ButtonGroup isAttached>
            <Button
              ...
              onClick={() => handleFilter("Tous", filter.status, filter.showConnectingDate)}
              px={10}
            >
              Tous
            </Button>
            {periods.map((period) => (
              <Button
                ...
                onClick={() => handleFilter(period, filter.status, filter.showConnectingDate)}
              >
                {period}
              </Button>
            ))}
          ...
          >
            {statuses.map((status) => (
              <Button
                ...
                onClick={() => handleFilter(filter.period, status, filter.showConnectingDate)}
                px={10}
              >
                {status}
              </Button>
        ...
        <Box mb={4}>
          <Button
            size="md"
            colorScheme={filter.showConnectingDate ? "blue" : "gray"}
            onClick={() => handleFilter(filter.period, filter.status, !filter.showConnectingDate)}
            px={10}
          >
